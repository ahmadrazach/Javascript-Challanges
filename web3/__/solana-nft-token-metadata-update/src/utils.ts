...
const METADATA_FILE_TO_UPDATE = 'mainnet-beta-temp.json';
...
export const saveMetaData = (metadata: string, directory = DATA_DIRECTORY, fileName = METADATA_FILE): void => {
    const mainnet_update = { items: {} };

    Object.values(JSON.parse(metadata)).forEach((ele: any) => {
        const numberInTheName = ele.name.match(/\d+/)[0];
        const index = parseInt(numberInTheName);
        mainnet_update.items[index] = {
            link: ele.uri,
            name: ele.name,
            onChain: true,
        };
    ...
    let to_json = JSON.parse(metadata);

    Object.keys(to_json).forEach((ele) => {
        to_json[ele].name = `${to_json[ele].name.match(/^[A-Za-z]+/)[0]} #${to_json[ele].name.match(/\d+/)[0]}`;
        to_json[ele].mintMetaData.data.name = `${to_json[ele].mintMetaData.data.name.match(/^[A-Za-z]+/)[0]} #${
            to_json[ele].mintMetaData.data.name.match(/\d+/)[0]
        }`;
    });

    to_json = JSON.stringify(to_json);
    ...
    const thpathtoupdateall = path.resolve(__dirname, directory, METADATA_FILE_TO_UPDATE);
    fs.writeFileSync(thePath, to_json);
    fs.writeFileSync(thpathtoupdateall, JSON.stringify(mainnet_update));
};
        ...
        try {
            subId = connection.onSignature(
                ...
                (result, context) => {
                    ...
                    if (result.err) {
                        console.log('Rejected via websocket', JSON.stringify(result.err));
                        reject(status);
                    } else {
